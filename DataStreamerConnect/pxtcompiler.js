                ...
                function getBinaryExpressionStatement(n) {
                    ...
                    switch (n.operatorToken.kind) {
                        ...
                        case SK.PlusEqualsToken:
                            if (isTextJoin(n)) {
                                var r_2 = mkStmt("variables_set");
                                var renamed = getVariableName(n.left);
                                trackVariableUsage(renamed, ReferenceType.InBlocksOnly);
                                r_2.inputs = [mkValue("VALUE", getTextJoin(n), numberType)];
                                r_2.fields = [getField("VAR", renamed)];
                                return r_2;
                            }
                    ...
                    if (info.qName == "Math.pow") {
                        ...
                        if (isSupportedMathFunction(op)) {
                            ...
                            if (isRoundingFunction(op)) {
                                r_4 = mkExpr("math_js_round");
                            }
                            else {
                                r_4 = mkExpr("math_js_op");
                                var opType = void 0;
                                if (isUnaryMathFunction(op))
                                    ...
                                    opType = "binary";
                                r_4.mutation = { "op-type": opType };
                            }
                            r_4.inputs = info.args.map(function (arg, index) { return mkValue("ARG" + index, getOutputBlock(arg), "math_number"); });
                            r_4.fields = [getField("OP", op)];
                            return r_4;
                        }
                    }
                    if (attributes.blockId === pxtc.PAUSE_UNTIL_TYPE) {
                        var r_5 = mkStmt(pxtc.PAUSE_UNTIL_TYPE);
                        ...
                        r_5.inputs = [mkValue("PREDICATE", getOutputBlock(condition), "logic_boolean")];
                        return r_5;
                    }
                    if (!attributes.blockId || !attributes.block) {
                        ...
                        if (!builtin) {
                            ...
                            if (env.declaredFunctions[name_2]) {
                                var r_6 = mkStmt("procedures_callnoreturn");
                                r_6.mutation = { "name": name_2 };
                                return r_6;
                            }
                            else {
    ...
    (function (pxtc) {
        ...
        function compileBinary(program, host, opts, res, entryPoint) {
            ...
            */
            ...
            function emitAsStringCore(e, tp, emitted) {
                if (pxtc.target.floatingPoint && (tp.flags & (ts.TypeFlags.NumberLike | ts.TypeFlags.Boolean | ts.TypeFlags.BooleanLiteral)))
                    return pxtc.ir.rtcallMask("numops::toString", 1, pxtc.ir.CallingConvention.Plain, [emitted]);
                else if (tp.flags & ts.TypeFlags.NumberLike)
                    return pxtc.ir.rtcall("Number_::toString", [emitted]);
                else if (isBooleanType(tp))
                    return pxtc.ir.rtcall("Boolean_::toString", [emitted]);
                else if (isStringType(tp))
                    return emitted; // OK
